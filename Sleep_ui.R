sidebarLayout(
  sidebarPanel (width = 3,
                ...
                fluidRow(
                  column(12,
                         sliderInput('yLimitsBoutSleepCount','Sleep bout count y limits',min = 0, max = 500, value = c(0,500)))),
                ...
                HTML('<h4><b> Sleep bout count </b></h4>'),

                fluidRow(
                  column(6, textInput("SleepBoutCountBoxTitle", "Sleep bout count title","")),
                  column(6, textInput("SleepBoutCountBoxYLabel", "Sleep bout count Y axis",""))),
                fluidRow(
                  column(6, textInput("SleepBoutCountBoxXLabel", "Sleep bout count X axis",""))
                ),
  ...
  mainPanel(
    ...
    h3("Sleep bout count", align = "center", style = "font-weight:bold; margin-bottom:10px; border-radius: 5pt;
       background-color:LightCyan; padding:7px "),

    tabsetPanel(id = "SleepBoutCountPlotsTabs",
                tabPanel("Sleep bout count per light phase",plotOutput("SleepBoutCountDayNight")),
                tabPanel("Sleep bout count per day",plotOutput("SleepBoutCountPerDay")),
                tabPanel("Sleep bout count per day and light phase",plotOutput("SleepBoutCountDayNightPerDay")),
                tabPanel("Sleep bout count customized time",plotOutput("SleepBoutCountCustomized"))

    ),
    fluidRow(
      column(8,DT::dataTableOutput("SleepBoutCountSummary"))),

    fluidRow(style = "margin-top:10px",
             column(2, style = "margin-top:20px",
                    downloadButton("saveSleepBoutCountFig", "Save image")),
             column(2,
                    selectInput("SleepBoutCountFig","",choices = c(".png",".jpg",".tiff"), selected = ".png")),
             column(2,),
             column(2, style = "margin-top:20px",
                    downloadButton("saveSleepBoutCountReport", "Save data")),
    )
  ))