...
statisticsReport <- function(Data){
  ...
  # Convert to data.frame if it's a data.table to avoid $ operator issues
  if (inherits(Data, "data.table")) {
    Data <- as.data.frame(Data)
  }

  # Remove rows with NaN or NA in yPlot before computing statistics
  yPlot_col <- Data[, 'yPlot']
  valid_rows <- !is.na(yPlot_col) & !is.nan(yPlot_col)
  Data <- Data[valid_rows, ]

  # Return empty data frame if no valid data
  if (nrow(Data) == 0) {
    return(data.frame(Condition = character(), Label = character(), N = integer(),
                     Data_Mean = numeric(), Data_SEM = numeric(), Data_SD = numeric(),
                     Data_Median = numeric(), Data_1st_Quartile = numeric(), Data_3rd_Quartile = numeric()))
  }

  conditions <- aggregate(yPlot ~ interaction(labels, xPlot,sep = " - "), data=Data, FUN=mean, na.rm=TRUE)[,1]

  Condition <- str_split_fixed(conditions, " - ",3)[,1]
  Label <-  str_split_fixed(conditions, " - ",3)[,2]

  N  <- aggregate(yPlot ~ interaction(labels, xPlot,sep = " - "), data=Data, FUN=function(x) sum(!is.na(x)))[,2]
  Data_Mean <- round(aggregate(yPlot ~ interaction(labels,xPlot, sep = " - "), data=Data, FUN=mean, na.rm=TRUE)[,2],3)
  Data_SEM <- round(aggregate(yPlot ~ interaction(labels, xPlot,sep = " - "), data=Data, FUN=se)[,2],3)
  Data_SD <- round(aggregate(yPlot ~ interaction(labels, xPlot,sep = " - "), data=Data, FUN=sd, na.rm=TRUE)[,2],3)
  Data_Median <- round(aggregate(yPlot ~ interaction(labels,xPlot, sep = " - "), data=Data, FUN=median, na.rm=TRUE)[,2],3)
  Data_1st_Quartile <- round(aggregate(yPlot ~ interaction(labels, xPlot,sep = " - "), data=Data, FUN=function(x) quantile(x, 0.25, na.rm=TRUE))[,2],3)
  Data_3rd_Quartile <- round(aggregate(yPlot ~ interaction(labels, xPlot,sep = " - "), data=Data, FUN=function(x) quantile(x, 0.75, na.rm=TRUE))[,2],3)

  result <- data.frame(Condition, Label, N, Data_Mean, Data_SEM, Data_SD, Data_Median, Data_1st_Quartile, Data_3rd_Quartile)

  # Replace NaN/Inf with NA for clean Excel export
...
dataReport <- function(data){
  ...
  File <- data[1:nrow(resultSplit), 'File']
  Labels <- data[1:nrow(resultSplit), 'labels']
  Channels <- data[1:nrow(resultSplit), 'region_id']
  Start_date <- data[1:nrow(resultSplit), 'start_datetime']
  End_date <- data[1:nrow(resultSplit), 'stop_datetime']
  ...
  # Replace NaN and Inf values with NA to avoid #NUM! errors in Excel
  exportData[] <- lapply(exportData, function(x) {
    if (is.numeric(x)) {
      x[is.nan(x) | is.infinite(x)] <- NA
    }
    return(x)
...
boutActivitySummary <- function(graphs){
  ...
  # Convert NaN values to 0 to maintain consistent N across all statistics
  # This ensures animals with no activity bouts are still counted
  if (any(is.nan(summaryDT_melted[,yPlot]))) {
    summaryDT_melted[is.nan(yPlot), yPlot := 0]
  }
