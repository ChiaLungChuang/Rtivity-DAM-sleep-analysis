...
Figures <- reactiveValues(actogram = NULL, doublePlot = NULL, chronogram = NULL, chronogram24h = NULL, cumActivity = NULL, sleep = NULL)
FiguresTitles <- reactiveValues(actogram = "Actogram", doublePlot = "Double Plot Actogram", chronogram = "Full chronogram", chronogram24h = "Average LD cycle chronogram", cumActivity = "Cumulative Activity", sleep = "Sleep Chronogram")
FiguresXlabels <- reactiveValues(ActivityAndSleep = paste0("Time (days)"))
FiguresYlabels <- reactiveValues(activity = paste("Activity per 60 minutes"), doublePlot = "Period", cumActivity = "Cumulative Activity", sleep = paste("Sleep ratio per 60 minutes"))

PeriodicData <- reactiveValues(activity = NULL, cumActivity = NULL, sleep = NULL)
PeriodicStatistics <- reactiveValues(activity = NULL, cumActivity = NULL, sleep = NULL)
...
update_periodT <- function(){
  ...
  if (is.numeric(input$x0)){
    ...
    # Convert Period objects to numeric seconds for R 4.5+ compatibility
    if (input$xTime == "days"){
      sumTime <- as.numeric(days(input$x0), unit = "secs")}
    if (input$xTime == "hour"){
      sumTime <- as.numeric(hours(input$x0), unit = "secs")}
    if (input$xTime == "min"){
      sumTime <- as.numeric(mins(input$x0), unit = "secs")}
    if (input$xTime == "sec"){
      sumTime <- input$x0}
...
plotLabels <- function(fig,graph){
  ...
  if (input$lightDark == TRUE){
    ...
    fig <- fig + stat_ld_annotations(l_duration = as.numeric(hours(floor(l_hours())), unit = "secs"), period = as.numeric(hours(l_period()), unit = "secs"))
  }
...
updateActogram  <- function () {
  ...
  # Ensure Data column exists before accessing
  req("Data" %in% names(damData$dt))

  # Convert Period to numeric seconds for R 4.5+ compatibility
  fig <- damData$dt %>%
    ggetho(aes(x = periodT, y = Data,z = activity),
                summary_FUN = sum,summary_time_window = as.numeric(mins(binSize()), unit = "secs"))
...
updateDoublePlot <- function() {
  ...
  # Convert Period to numeric seconds for R 4.5+ compatibility
  fig <- damData$dt %>%
    ggetho(aes(x = t, z = activity),multiplot = 2, multiplot_period = as.numeric(hours(l_period()), unit = "secs"), summary_FUN = sum,
                summary_time_window = as.numeric(mins(binSize()), unit = "secs")) + facet_wrap( ~ Data)
...
updateChronogram <- function() {
  ...
  fig <- damData$dt %>%
    ggetho(aes(x = periodT, y = activity, colour=Data, linetype = Data),summary_FUN = sum,
                summary_time_window = as.numeric(mins(binSize()), unit = "secs")) + stat_pop_etho()
...
updateChronogram24h <- function(){
  ...
  fig <- damData$dt %>%
    ggetho(aes(x = periodT, y = activity24h, colour=Data, linetype = Data),summary_FUN = sum,
                summary_time_window = as.numeric(mins(binSize()), unit = "secs"), time_wrap = as.numeric(hours(l_period()), unit = "secs"))+stat_pop_etho()
...
updateFigures <- function(){
  ...
  # Ensure Data column exists before generating figures
  req("Data" %in% names(damData$dt))
...
#Statistics Data
periodicFigures <- function(graph = 'all'){
  ...
  withProgress(message = 'Actogram and Chronogram analysis', value = 0, {
    ...
    if (graph == "sleep"){
      ...
      for (i in 1:length(start)){
        ...
        File <-c(File,rep(damData$dt[start[i],file],n))
        Labels <- c(Labels,rep(damData$dt[start[i],labels],n))
        Order <- c(Order,rep(damData$dt[start[i],order],n))
        Channels <- c(Channels, rep(damData$dt[start[i],channels],n))
        Start_date <- c(Start_date, rep(damData$dt[,start_datetime,meta=T][i],n))
        End_date <- c(End_date, rep(damData$dt[,stop_datetime,meta=T][i],n))
      ...
      PeriodicData$sleep <- PeriodicData$sleep[order(Order),]
      ##### Organize data in sleep per animal #####
      ...
      PeriodicData$activity <- PeriodicData$activity[order(Order),]
      PeriodicData$cumActivity <- PeriodicData$cumActivity[order(Order),]
      PeriodicData$sleep <- PeriodicData$sleep[order(Order),]
      ##### Organize data in activity per animal #####
...
observeEvent(input$sleepAnalysis,{
  ...
  # Update sleep annotations and sleep sliders
...
output$saveData <- downloadHandler(
  ...
  content = function(file){
    ...
    tryCatch({
      ...
      if(input$tabs == "Sleep Chronogram"){
        ActivityRepresentationsData(graph = "sleep")
      ...
      if (is.null(data) || nrow(data) == 0) {
        showNotification("No data available to save. Please ensure graphs are generated first.", type = "error")
        return(NULL)
      }
    ...
    addWorksheet(wb, "All_Channels_Horizontal")
    writeData(wb, "All_Channels_Horizontal", summaryData)
