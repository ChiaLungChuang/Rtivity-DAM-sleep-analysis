...
PeriodogramData <- function() {
  ...
  # Convert Period to numeric seconds for R 4.5+ compatibility in comparison
  if(max(damData$dt[,t])-min(damData$dt[,t]) > as.numeric(hours(l_period()), unit = "secs") & input$minPer<l_period() & input$maxPer > l_period()){
...
DAFsummary <- function(){
  ...
  if (input$twoScaling == TRUE){
    for(i in 1:length(ids)){
      ...
      animalDFA1 <- dfa(time.series = animalData[,activity],npoints = input$nFractal,
                        window.size.range=c(as.numeric(mins(input$Fractal_limits[1]), unit = "secs"),as.numeric(mins(input$TimeScale_limits), unit = "secs")),
                        do.plot=FALSE)
      ...
      animalDFA2 <- dfa(time.series = animalData[,activity],npoints = input$nFractal,
                        window.size.range=c(as.numeric(mins(input$TimeScale_limits), unit = "secs"),as.numeric(mins(input$Fractal_limits[2]), unit = "secs")),
                        do.plot=FALSE)
  ...
  DAFdata$ScalingExponents <- data.frame(File, Labels, Channels, Start_datetime, End_datetime, ScalingExponent1, ScalingExponent2)
    ...
    for(i in 1:length(ids)){
      ...
      # Convert Period to numeric seconds for R 4.5+ compatibility
      animalDFA1 <- dfa(time.series = animalData[,activity],npoints = input$nFractal,
                        window.size.range=c(as.numeric(mins(input$Fractal_limits[1]), unit = "secs"),as.numeric(mins(input$Fractal_limits[2]), unit = "secs")),
                        do.plot=FALSE)
...
observe({
  ...
  output$savePeriodogramData <- downloadHandler(
    ...
    content = function(file){
      ...
      addWorksheet(wb, "Period Statistics")
      writeData(wb, "Period Statistics", CircadianStatistics$period)

      addWorksheet(wb, "Power Statistics")
      writeData(wb, "Power Statistics", CircadianStatistics$power)
      ...
      writeData(wb, "All channels", exportAllChannels)
      ...
      writeData(wb, "Period", periodData)
      addWorksheet(wb, "Power")
      writeData(wb, "Power", powerData)
...
observe({
  ...
  output$savePhaseData <- downloadHandler(
    ...
    content = function(file){
      ...
      writeData(wb, "Statistics", PhaseReport(data))
...
observe({
  ...
  output$saveFractalReport <- downloadHandler(
    ...
    content = function(file){
      ...
      addWorksheet(wb, "Statistics")
      writeData(wb, "Statistics", DAFstatisticsData())

      addWorksheet(wb, "All data")
      writeData(wb, "All data", DAFdata$ScalingExponents)
