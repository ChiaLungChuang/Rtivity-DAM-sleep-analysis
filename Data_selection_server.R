...
dateTime <- reactive({
  tryCatch({
    ...
    # Parse as UTC — wall-clock times, never shifted by local timezone
    parsed_dateTime <- lubridate::dmy_hms(datetime_str, tz = "UTC")
...
updateDates <- function(){
  # Silent guard: nothing to update until files are loaded
  if (is.null(ImportedFiles()) || length(ImportedFiles()) == 0) return()
  ...
  tryCatch({
    ...
    # Safely get start and stop datetimes (stored POSIXct can come back as numeric)
    raw_start <- tryCatch_wrapper(dates$startDateTime[n], "Error getting start datetime")
    raw_stop  <- tryCatch_wrapper(dates$stopDateTime[n], "Error getting stop datetime")

    if(is.null(raw_start) || is.null(raw_stop)) {
      log_error("Start or stop datetime is NULL")
      return()
    }

    startDateTime <- if (is.numeric(raw_start)) as.POSIXct(raw_start, origin = "1970-01-01", tz = "UTC") else as.POSIXct(raw_start, tz = "UTC")
    stopDateTime  <- if (is.numeric(raw_stop))  as.POSIXct(raw_stop,  origin = "1970-01-01", tz = "UTC") else as.POSIXct(raw_stop,  tz = "UTC")

    # Update date inputs
    updateDateInput(session, "Start_date", value = as.Date(format(startDateTime, tz = "UTC")))
    updateDateInput(session, "Finish_date", value = as.Date(format(stopDateTime,  tz = "UTC")))

    # Update time text inputs — format in UTC so the hour is never shifted
    updateTextInput(session, "start_time",  value = format(startDateTime, "%H:%M", tz = "UTC"))
    updateTextInput(session, "finish_time", value = format(stopDateTime,  "%H:%M", tz = "UTC"))

    log_info(paste("Updated start datetime:", format(startDateTime, "%Y-%m-%d %H:%M:%S", tz = "UTC")))
    log_info(paste("Updated stop datetime:", format(stopDateTime, "%Y-%m-%d %H:%M:%S", tz = "UTC")))

    # Update zeitgeber time text input
    zt_raw <- zeitgeberTimes()[n]
    if (!is.null(zt_raw)) {
      zt_hhmm <- substr(zt_raw, 1, 5)  # "08:00:00" → "08:00"
      updateTextInput(session, "zeitgeberTime", value = zt_hhmm)
    }
...
observeEvent(input$files, {
  tryCatch_wrapper({
    ...
    withProgress(message = 'Importing files', value = 0, {
      ...
      # Initialize dates from actual file data
      day_col   <- table[, 2]
      month_col <- table[, 3]
      year_col  <- table[, 4]
      time_col  <- table[, 5]
      parsed_dt <- lubridate::dmy_hms(paste(paste(day_col, month_col, year_col, sep = "-"), time_col), tz = "UTC")

      for (i in seq_along(import)) {
        start_idx <- if (i == 1) 1 else thresholds$l[i - 1] + 1
        end_idx   <- thresholds$l[i]
        dates$startDateTime[i] <- parsed_dt[start_idx]
        dates$stopDateTime[i]  <- parsed_dt[end_idx]
      }
...
observeEvent(input$zeitgeberTime,{
  ...
  zts <- zeitgeberTimes()
  # input$zeitgeberTime is now a text string "HH:MM"
  zt_str <- trimws(input$zeitgeberTime)
  new_time <- if (grepl("^[0-9]{1,2}:[0-9]{2}$", zt_str)) paste0(zt_str, ":00") else "08:00:00"
...
observeEvent(changeDate(), {
  # Silent guard: do nothing until files are actually loaded
  if (is.null(ImportedFiles()) || is.null(input$selectedFile) || input$selectedFile == "" || input$selectedFile == "-") return()
  ...
  tryCatch({
    ...
    # Parse HH:MM text inputs into a time string (no timezone — plain wall-clock numbers)
    parse_hhmm <- function(s, fallback) {
      s <- trimws(as.character(s))
      if (grepl("^[0-9]{1,2}:[0-9]{2}$", s))          paste0(s, ":00")
      else if (grepl("^[0-9]{1,2}:[0-9]{2}:[0-9]{2}$", s)) s
      else fallback
    }
    start_time  <- parse_hhmm(input$start_time,  "00:00:00")
    finish_time <- parse_hhmm(input$finish_time, "23:59:59")

    # Combine date + time as UTC so format() never shifts the hour
    start  <- as.POSIXct(paste(as.character(input$Start_date),  start_time),
                         format = "%Y-%m-%d %H:%M:%S", tz = "UTC")
    finish <- as.POSIXct(paste(as.character(input$Finish_date), finish_time),
                         format = "%Y-%m-%d %H:%M:%S", tz = "UTC")

    if (is.na(start) || is.na(finish)) {
      log_error("Failed to parse start or finish datetime")
      showNotification("Invalid date/time — use YYYY-MM-DD and HH:MM", type = "error")
      return()
    }

    # Only check that start is before finish; no clamping to data range
    if (start >= finish) {
      showNotification("Start date/time must be earlier than finish date/time",
                       type = "error", duration = 10)
      return()
    }

    log_info(paste("Parsed start:", format(start, tz = "UTC"), "finish:", format(finish, tz = "UTC")))
...
observeEvent(input$addcondition,{
  ...
  # Helper: convert any stored datetime value to a UTC wall-clock string
  to_utc_string <- function(dt) {
    if (inherits(dt, "POSIXt")) {
      format(dt, "%Y-%m-%d %H:%M:%S", tz = "UTC")
    } else if (is.numeric(dt) || (is.character(dt) && grepl("^[0-9.]+$", dt))) {
      format(as.POSIXct(as.numeric(dt), origin = "1970-01-01", tz = "UTC"),
             "%Y-%m-%d %H:%M:%S", tz = "UTC")
    } else {
      as.character(dt)
    }
  }
  start_datetime <- to_utc_string(start_dt)
  stop_datetime  <- to_utc_string(stop_dt)
...
observeEvent(input$importConditions,{
  ...
  ### Turn dates into date and time format (tz="UTC" — no local offset applied)
  table[,2] <- as.POSIXct(table[,2], tz = "UTC")
  table[,3] <- as.POSIXct(table[,3], tz = "UTC")
...
observeEvent(input$startanalysis, {
  ...
  withProgress(message = 'Importing data (1/2)', value = 0, {
    ...
    for(i in 1:nrow(Conditions$df)) {
      ...
      # If it's numeric or a numeric string, convert it (tz="UTC" keeps wall-clock time)
      if(is.numeric(start_val) || (is.character(start_val) && grepl("^[0-9.]+$", start_val))) {
        Conditions$df[i, 'start_datetime'] <- format(as.POSIXct(as.numeric(start_val), origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S", tz = "UTC")
      }
      if(is.numeric(stop_val) || (is.character(stop_val) && grepl("^[0-9.]+$", stop_val))) {
        Conditions$df[i, 'stop_datetime'] <- format(as.POSIXct(as.numeric(stop_val), origin = "1970-01-01", tz = "UTC"), "%Y-%m-%d %H:%M:%S", tz = "UTC")
      }
    }
  ...
  withProgress(message = 'Computing activity and sleep parameters (2/2)', value = 0, {
    ...
    for (file_idx in 1:length(ImportedFiles())) {
      # Get start time in UTC (wall-clock) — must match how datetimes are stored
      start_time_str <- strftime(dates$startDateTime[file_idx], format = "%H:%M:00", tz = "UTC")
      diff_val <- as.numeric(difftime(
        as.POSIXct(start_time_str, format = "%H:%M:%S", tz = "UTC"),
        as.POSIXct(zt_times[file_idx], format = "%H:%M:%S", tz = "UTC"),
        units = "mins"
      ))
