...
launcher <- c(
  ...
  '',
  '# Runtime data (log + Chrome profile) must live OUTSIDE the .app bundle.',
  '# macOS (TCC) blocks apps from writing inside protected folders such as',
  '# ~/Desktop, ~/Documents and ~/Downloads. When the bundle sits on the Desktop,',
  '# writing the log inside it fails with "Operation not permitted" and R never',
  '# starts. ~/Library is always writable by the app; keeping runtime data out of',
  '# the bundle also stops the bundle from bloating and keeps it read-only-clean.',
  'APP_SUPPORT="$HOME/Library/Application Support/Rtivity"',
  'mkdir -p "$APP_SUPPORT"',
  'LOG="$APP_SUPPORT/rtivity.log"',
  '',
  ...
  'PROFILE="$APP_SUPPORT/chrome-profile"',
  ...
  '    pkill -f "Rtivity/chrome-profile" 2>/dev/null ) &',
  '  WATCHDOG=$!',
  '',
